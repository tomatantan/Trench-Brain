---
type: entity
kind: player
title: @golocojp
updated: 2026-06-22
tags: [trench, entity, player]
posts: 36
---

# @golocojp

> 自動生成。信号投稿 36件。watchlist: [[watchlist]]。

## call track-record（[[manipulation-playbook]]・[[KOL track-record]]）
CA言及 8件 / 現outcome評価 6件中 **死0（0%）** ＝callが残りやすい(相対的に注目)。
> ★近似(現mcap基準)・小N。母集団は[[launchpad-economics]]で大半死＝相対比較で読む。

## よく言及するトークン
[[$CARDS]](2) [[$VEN]](1) [[$REYA]](1) [[$HYPE]](1) [[$POKERX]](1) [[$TCG]](1) [[$CPX]](1)

## 高エンゲージ投稿
| likes | tickers | 抜粋 | source |
|---|---|---|---|
| 20 |  | solana:EmcxFTNVDqyLHp11NvwvLZ4D7LKGbG9i7B8RF7dwpump @c0mputeAI FDV1.7Mでトップブラストして良かった | [[golocojp__2068244837426696341]] |
| 18 |  | これぞまさにGud Tek！！  leyten氏に賭けたい、応援の気持ちで solana:EmcxFTNVDqyLHp11NvwvLZ4D7LKGbG9i7B8RF7d | [[golocojp__2067702170162798877]] |
| 18 |  | kickstart szn  Still too early. | [[golocojp__2072244779363319999]] |
| 17 |  | solana:bS532krUcXBMNqXURPtGqYA7dhEsenYe2z9QKkcEASY @cpx_token  R6さんもdipでinして倍々👏  Cog | [[golocojp__2071971847340720472]] |
| 13 | [[$CPX]] | The MCG effects hit $CPX @cpx_token on @EasyA_Kickstart as usual even BTC going down | [[golocojp__2071974713656770786]] |
| 10 |  | .@seyong Thanks for building @fomo  Not only we can enjoy day trading in multi-chain | [[golocojp__2069870809573896194]] |
| 7 |  | Ornn     for Compute. solana:AWGCDT2gd8JadbYbYyZy1iKxfWokPNgrEQoU24zUpump for Memory | [[golocojp__2070074350750076989]] |
| 7 |  | EasyA x fomo....なんだ？？👀 | [[golocojp__2069795779494244617]] |
| 6 |  | 今回のNansen @nansen_ai の宝くじキャンペーンのルール変更は大大大朗報。  Solanaの現物少額デイトレしてても1-2枚/週はいけるだろうし、1000 | [[golocojp__2072008454903050342]] |
| 6 |  | if I win(1/4000)😂, I'll do giveaway(with hodl term) in some easyA ecos tokens to Jap | [[golocojp__2072425337875775558]] |

<!-- synthesis:start -->
## 思考の型（how they reason・視点エンジン用）

---

**lens（何を見るか）**
- **ナラティブ × エコシステム適合性**が一貫した着目軸。単体トークンより「pumpfun上のDeAI ETF構成」「tech ecosystemとして良い形か」という**バスケット/テーマ単位**で見る傾向。
- **インフラ層**にも注目（分散compute、GLM on WAN、knowledge graph）＝AI×crypto文脈を早期に拾う。
- 観測: ほぼ全投稿がRT＝**自分は発信者でなく増幅器**。何を増幅するかが選択眼の本体。

**style / edge（どう読み・賭けるか）**
- **KOL/narrative追随型**。kwok、fibonacki、sebbsssss等の確立したSolanaエコシステム人物を増幅→ **自前のオリジナル分析より「誰が言っているか」のシグナルを重視**。
- エコシステムトークンを「長期でETF的に持つ」フレーミング（"over the long term"）→ 魔界にしては保有継続の美学がある。
- 0%死（評価5件）はサンプル小だが、**ナラティブが弱い or 信頼できるKOLが推していないものを避けている**可能性がある。

**stance / thesis（核の主張）**
- **AI×Solana ecosystem（特にpumpfun eco上のDeAI）が本命**というbias。launchpad meta（kwok's launchpad、EasyA Kickstart）も強気。
- 日本語投稿（ofasya_のRT）が興味深い：「今までの魔界とは投機対象の分母が違いすぎる」→ **市場の構造変化を意識しており、過去のセオリーをそのまま適用しない自覚がある**（メタ認知）。
- ⚠️ **ポジトーク警戒**: $VSK、$CPT等を複数KOL経由でRTしており、ポジション後の増幅か事前調査かは判別不可。CA評価数が少なく、track-recordはまだ統計的に薄い。

**tells（この人の発言/callが意味するもの）**
- 「tech ecosystemとしての形」を評価するRTをした＝単発pump狙いでなく、**複数ピースが揃っている/揃いそうなエコシステム**に入ろうとしているシグナル。
- Harvard/credentials系の narrativeに乗りやすい（$VSKを複数KOL経由でRT）→ **ファンダメンタル narrative + 著名人連鎖**が重なった時に動く型。
- 一方、純粋なon-chain資金フローや板の変動に言及する投稿はほぼなし＝**オンチェーンで独自判断するプレイヤーではない**（KOL収束シグナルで入る）。

**視点エンジンでの使い方**
「この player なら、ある銘柄をどう一言で読むか」の雛形：

> **「AI x Solanaecoのテーマに乗っているか、確立KOLが複数収束しているか」でフィルタする。それが揃えばエコシステムETF的に入る。単発hypeや板だけで動かない。死0%はナラティブ強度フィルタが機能している可能性。ただしサンプル5=過信禁物。」**
<!-- synthesis:end -->
