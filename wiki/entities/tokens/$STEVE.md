---
type: entity
kind: token
source: auto-track
status: dead
title: $STEVE（The Ultimate Gambler）
mint: 2MGMmzgs5T66QPzymGWmPpHmVdtzTZ1AWZ1y45Fzpump
pool: F1mhxqYAEPqXwUpfP4mVcUxAmvJ7vY92GFAb3t4A1eWC
created: 2026-06-29
updated: 2026-06-29
tags: [trench, entity, token, auto-track, watch]
---

# $STEVE（The Ultimate Gambler）

## ライフサイクル(auto-track)
| 項目 | 値 |
|---|---|
| mint | 2MGMmzgs5T66QPzymGWmPpHmVdtzTZ1AWZ1y45Fzpump |
| pool | F1mhxqYAEPqXwUpfP4mVcUxAmvJ7vY92GFAb3t4A1eWC |
| gate | safety:ok / traction:graduated |
| mcap(birth観測) | $20,966（2026-06-28T18:18Z） |
| peak_mcap | $20,966（暫定） |
| real_sol | 0 |
| reply_count | 0 |
| twitter | https://x.com/Supermanonchain/status/2071296439930782133 |
| website | null |
| tokenized_agent | false |
| status | watch |
| auto-track birth | 2026-06-28T18:18Z |

<!-- synthesis:start -->
## 合成メモ（synthesis / エージェント記述）

### 2026-06-29 初回合成（auto-track birth）

**観測（事実）**
- pump.fun 産・graduated(complete=true)・2026-06-28T18:18Z 検知。
- mcap $20,966。real_sol=0・reply_count=0・kol_ca 空。
- twitter: @Supermanonchain。website=null。tokenized_agent=false。

**動線・型**
- [[launchpad-economics]]: "The Ultimate Gambler" + ticker STEVE = 汎用人名 × ギャンブル narrative。同バッチに $TOGI "The Gambling Bull"・$RISK "Take The Risk" とギャンブル系 meme が複数（需要分散可能性）。
- real_sol=0 × reply=0 × @Supermanonchain（未知アカウント）= 情報ゼロに近いスタブ水準。

**賭け仮説**（confidence=最低）
- 汎用人名・低 mcap・traction0 = [[rug-anatomy]] 標準崩壊候補。差別化なし。

### 2026-06-29 死亡確定（auto-track death）
**観測（事実）**
- last mcap $3,884（2026-06-28T21:35Z）・peak $26,506 比 **-85.3%**。cause: "mcap枯れ"。

**死因の型**
- 汎用人名 × ギャンブル系命名群（同バッチ $TOGI/$RISK/$BOTTOM/$TOPG）全滅。需要分散 + traction0 = 型通り。outcome: dead。
<!-- synthesis:end -->
