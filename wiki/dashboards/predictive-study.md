...
> `brain/predictive_study.py`。母集団 N=163（dead 106 / tracked 57＝**pendingでまだ死にうる=death率は下限**）。
> baseline 死亡率 65%。各因子はこれとの比較で読む。★現mcap近似・N小cell留意・同一launchpad/近時間で独立性限定。

## 死の深さ: dead銘柄の平均drawdown **-49.7%**（peak比）＝死は「フェード」でなく「崩落」
...
| 因子 | 死亡率 |
|---|---|
| traction無 | 102/153 (67%) |
| traction有 | 4/10 (40%) |

### ③ ★交互作用 gate×traction（最も効く組合せ）
| 因子 | 死亡率 |
|---|---|
| graduated × tr無 | 92/105 (88%) |
...
- **peak mcap <10k ＝ 死53/53 (100%)**＝最も clean な死signal（小peakは事実上全滅・mcap勢いが立たない銘柄は乗らない）。
- **最悪の gate×traction**: graduated × tr無＝死92/105 (88%)＝最強のavoid。
- **最良の gate×traction**: mcap勢い門 × tr無＝死6/39 (15%)＝相対的に生存（[[manipulation-playbook]]で偽traction除外が前提）。
