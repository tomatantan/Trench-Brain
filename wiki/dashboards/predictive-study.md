...
> `brain/predictive_study.py`。母集団 N=122（dead 84 / tracked 38＝**pendingでまだ死にうる=death率は下限**）。
> baseline 死亡率 69%。各因子はこれとの比較で読む。★現mcap近似・N小cell留意・同一launchpad/近時間で独立性限定。

## 死の深さ: dead銘柄の平均drawdown **-54.6%**（peak比）＝死は「フェード」でなく「崩落」
...
- **最悪の gate×traction**: graduated × tr無＝死70/79 (89%)＝最強のavoid。
