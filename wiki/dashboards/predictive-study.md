...
> `brain/predictive_study.py`。母集団 N=107（dead 74 / tracked 33＝**pendingでまだ死にうる=death率は下限**）。
> baseline 死亡率 69%。各因子はこれとの比較で読む。★現mcap近似・N小cell留意・同一launchpad/近時間で独立性限定。

## 死の深さ: dead銘柄の平均drawdown **-54.7%**（peak比）＝死は「フェード」でなく「崩落」
...
- **peak mcap <10k ＝ 死33/33 (100%)**＝最も clean な死signal（小peakは事実上全滅・mcap勢いが立たない銘柄は乗らない）。
- **最悪の gate×traction**: graduated × tr無＝死62/68 (91%)＝最強のavoid。
- **最良の gate×traction**: mcap勢い門 × tr無＝死4/20 (20%)＝相対的に生存（[[manipulation-playbook]]で偽traction除外が前提）。
