...
> `brain/predictive_study.py`。母集団 N=21（dead 8 / tracked 13＝**pendingでまだ死にうる=death率は下限**）。
> baseline 死亡率 38%。各因子はこれとの比較で読む。★現mcap近似・N小cell留意・同一launchpad/近時間で独立性限定。

## 死の深さ: dead銘柄の平均drawdown **-14.9%**（peak比）＝死は「フェード」でなく「崩落」
...
- **peak mcap <10k ＝ 死7/7 (100%)**＝最も clean な死signal（小peakは事実上全滅・mcap勢いが立たない銘柄は乗らない）。
