...
> `brain/predictive_study.py`。母集団 N=106（dead 70 / tracked 36＝**pendingでまだ死にうる=death率は下限**）。
> baseline 死亡率 66%。各因子はこれとの比較で読む。★現mcap近似・N小cell留意・同一launchpad/近時間で独立性限定。

## 死の深さ: dead銘柄の平均drawdown **-53.6%**（peak比）＝死は「フェード」でなく「崩落」
...
- **peak mcap <10k ＝ 死32/32 (100%)**＝最も clean な死signal（小peakは事実上全滅・mcap勢いが立たない銘柄は乗らない）。
- **最悪の gate×traction**: graduated × tr無＝死59/67 (88%)＝最強のavoid。
- **最良の gate×traction**: mcap勢い門 × tr無＝死3/20 (15%)＝相対的に生存（[[manipulation-playbook]]で偽traction除外が前提）。
