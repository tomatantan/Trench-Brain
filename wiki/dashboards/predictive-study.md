...
> `brain/predictive_study.py`。母集団 N=84（dead 56 / tracked 28＝**pendingでまだ死にうる=death率は下限**）。
> baseline 死亡率 67%。各因子はこれとの比較で読む。★現mcap近似・N小cell留意・同一launchpad/近時間で独立性限定。

## 死の深さ: dead銘柄の平均drawdown **-52.2%**（peak比）＝死は「フェード」でなく「崩落」
...
- **最悪の gate×traction**: graduated × tr無＝死45/49 (92%)＝最強のavoid。
