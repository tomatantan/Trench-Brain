...
> `brain/predictive_study.py`。母集団 N=125（dead 85 / tracked 40＝**pendingでまだ死にうる=death率は下限**）。
> baseline 死亡率 68%。各因子はこれとの比較で読む。★現mcap近似・N小cell留意・同一launchpad/近時間で独立性限定。

## 死の深さ: dead銘柄の平均drawdown **-55.0%**（peak比）＝死は「フェード」でなく「崩落」
...
- **peak mcap <10k ＝ 死38/38 (100%)**＝最も clean な死signal（小peakは事実上全滅・mcap勢いが立たない銘柄は乗らない）。
- **最悪の gate×traction**: graduated × tr無＝死71/79 (90%)＝最強のavoid。
- **最良の gate×traction**: mcap勢い門 × tr無＝死6/27 (22%)＝相対的に生存（[[manipulation-playbook]]で偽traction除外が前提）。
