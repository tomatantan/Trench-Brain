...
> `brain/predictive_study.py`。母集団 N=39（dead 21 / tracked 18＝**pendingでまだ死にうる=death率は下限**）。
> baseline 死亡率 54%。各因子はこれとの比較で読む。★現mcap近似・N小cell留意・同一launchpad/近時間で独立性限定。

## 死の深さ: dead銘柄の平均drawdown **-28.6%**（peak比）＝死は「フェード」でなく「崩落」
...
- **peak mcap <10k ＝ 死15/15 (100%)**＝最も clean な死signal（小peakは事実上全滅・mcap勢いが立たない銘柄は乗らない）。
- **最悪の gate×traction**: graduated × tr無＝死13/16 (81%)＝最強のavoid。
