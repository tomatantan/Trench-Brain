...
## 母集団: tracked 210件（dead 131 / pending(tracked) 79）

## 型の hit-rate（観測）

| 型(仮説) | 検証 | 判定 |
|---|---|---|
| **traction無し→死ぬ** | 死亡 122/179 (68%)・残り57はpending | 支持(死多) |
| **traction有り→生存** | traction有り母集団 N=31（死9） | 検証可 |
| gate=graduated の死亡率 | 112/127 (88%) | 観測 |
| gate=mcap勢い門 の死亡率 | 9/46 (19%) | 観測 |
| gate=other の死亡率 | 10/37 (27%) | 観測 |

## ⚠️ 計測の限界（正直に）
- pending(tracked)が79件＝まだ生死未決着＝hit-rateは暫定（決着で更新）。
