...
## 母集団: tracked 63件（dead 44 / pending(tracked) 19）

## 型の hit-rate（観測）

| 型(仮説) | 検証 | 判定 |
|---|---|---|
| **traction無し→死ぬ** | 死亡 40/53 (75%)・残り13はpending | 支持(死多) |
| **traction有り→生存** | traction有り母集団 N=10（死4） | 検証可 |
| gate=graduated の死亡率 | 35/35 (100%) | 観測 |
| gate=mcap勢い門 の死亡率 | 1/9 (11%) | 観測 |
| gate=other の死亡率 | 8/19 (42%) | 観測 |

## ⚠️ 計測の限界（正直に）
- pending(tracked)が19件＝まだ生死未決着＝hit-rateは暫定（決着で更新）。
