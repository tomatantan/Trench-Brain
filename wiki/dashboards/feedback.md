...
## 母集団: tracked 125件（dead 85 / pending(tracked) 40）

## 型の hit-rate（観測）

| 型(仮説) | 検証 | 判定 |
|---|---|---|
| **traction無し→死ぬ** | 死亡 79/113 (69%)・残り34はpending | 支持(死多) |
| **traction有り→生存** | traction有り母集団 N=12（死6） | 検証可 |
| gate=graduated の死亡率 | 71/79 (89%) | 観測 |
| gate=mcap勢い門 の死亡率 | 6/27 (22%) | 観測 |
| gate=other の死亡率 | 8/19 (42%) | 観測 |

## ⚠️ 計測の限界（正直に）
- pending(tracked)が40件＝まだ生死未決着＝hit-rateは暫定（決着で更新）。
