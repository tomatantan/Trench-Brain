...
## 母集団: tracked 207件（dead 127 / pending(tracked) 80）

## 型の hit-rate（観測）

| 型(仮説) | 検証 | 判定 |
|---|---|---|
| **traction無し→死ぬ** | 死亡 118/176 (67%)・残り58はpending | 支持(死多) |
| **traction有り→生存** | traction有り母集団 N=31（死9） | 検証可 |
| gate=graduated の死亡率 | 109/124 (87%) | 観測 |
| gate=mcap勢い門 の死亡率 | 8/46 (17%) | 観測 |
| gate=other の死亡率 | 10/37 (27%) | 観測 |

## ⚠️ 計測の限界（正直に）
- pending(tracked)が80件＝まだ生死未決着＝hit-rateは暫定（決着で更新）。
