...
## 母集団: tracked 178件（dead 120 / pending(tracked) 58）

## 型の hit-rate（観測）

| 型(仮説) | 検証 | 判定 |
|---|---|---|
| **traction無し→死ぬ** | 死亡 114/161 (70%)・残り47はpending | 支持(死多) |
| **traction有り→生存** | traction有り母集団 N=17（死6） | 検証可 |
| gate=graduated の死亡率 | 104/114 (91%) | 観測 |
| gate=mcap勢い門 の死亡率 | 8/40 (20%) | 観測 |
| gate=other の死亡率 | 8/24 (33%) | 観測 |

## ⚠️ 計測の限界（正直に）
- pending(tracked)が58件＝まだ生死未決着＝hit-rateは暫定（決着で更新）。
