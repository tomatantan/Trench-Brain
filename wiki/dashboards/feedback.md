...
## 母集団: tracked 77件（dead 53 / pending(tracked) 24）

## 型の hit-rate（観測）

| 型(仮説) | 検証 | 判定 |
|---|---|---|
| **traction無し→死ぬ** | 死亡 49/67 (73%)・残り18はpending | 支持(死多) |
| **traction有り→生存** | traction有り母集団 N=10（死4） | 検証可 |
| gate=graduated の死亡率 | 42/44 (95%) | 観測 |
| gate=mcap勢い門 の死亡率 | 3/14 (21%) | 観測 |
| gate=other の死亡率 | 8/19 (42%) | 観測 |

## ⚠️ 計測の限界（正直に）
- pending(tracked)が24件＝まだ生死未決着＝hit-rateは暫定（決着で更新）。
