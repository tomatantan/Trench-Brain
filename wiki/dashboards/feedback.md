...
## 母集団: tracked 116件（dead 77 / pending(tracked) 39）

## 型の hit-rate（観測）

| 型(仮説) | 検証 | 判定 |
|---|---|---|
| **traction無し→死ぬ** | 死亡 71/104 (68%)・残り33はpending | 支持(死多) |
| **traction有り→生存** | traction有り母集団 N=12（死6） | 検証可 |
| gate=graduated の死亡率 | 65/74 (87%) | 観測 |
| gate=mcap勢い門 の死亡率 | 4/23 (17%) | 観測 |
| gate=other の死亡率 | 8/19 (42%) | 観測 |

## ⚠️ 計測の限界（正直に）
- pending(tracked)が39件＝まだ生死未決着＝hit-rateは暫定（決着で更新）。
