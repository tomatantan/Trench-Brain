...
## 母集団: tracked 89件（dead 42 / pending(tracked) 47）

## 型の hit-rate（観測）

| 型(仮説) | 検証 | 判定 |
|---|---|---|
| **traction無し→死ぬ** | 死亡 40/72 (55%)・残り32はpending | 支持(死多) |
| **traction有り→生存** | traction有り母集団 N=17（死2） | 検証可 |
| gate=graduated の死亡率 | 37/49 (75%) | 観測 |
| gate=mcap勢い門 の死亡率 | 3/22 (13%) | 観測 |
| gate=other の死亡率 | 2/18 (11%) | 観測 |

## ⚠️ 計測の限界（正直に）
- pending(tracked)が47件＝まだ生死未決着＝hit-rateは暫定（決着で更新）。
