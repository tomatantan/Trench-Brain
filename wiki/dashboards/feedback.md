...
## 母集団: tracked 37件（dead 17 / pending(tracked) 20）

## 型の hit-rate（観測）

| 型(仮説) | 検証 | 判定 |
|---|---|---|
| **traction無し→死ぬ** | 死亡 14/27 (51%)・残り13はpending | 支持(死多) |
| **traction有り→生存** | traction有り母集団 N=10（死3） | 検証可 |
| gate=graduated の死亡率 | 9/14 (64%) | 観測 |
| gate=mcap勢い門 の死亡率 | 1/4 (25%) | 観測 |
| gate=other の死亡率 | 7/19 (36%) | 観測 |

## ⚠️ 計測の限界（正直に）
- pending(tracked)が20件＝まだ生死未決着＝hit-rateは暫定（決着で更新）。
