...
| C4 | 指針4 | 1ソース取込→複数ページに波及(synthesis) | ✅PASS | 波及指示あり・summary3+entity657枚 |
| C5 | 指針5 | 矛盾は消さず⚠️両論で保持する | ✅PASS | concept内 ⚠️/矛盾 88箇所・prompt指示あり |
| C6 | 指針6 | entityで観測(事実)と推論(判断)を分離する | ✅PASS | 観測/判断を分離した entity 583/657枚 |
...
| R3c | 指針3/6 死の計測整合 | 死の分母counter(base_rate.died)が実dead数(tracked.json)と一致＝正しく数えてる | ✅PASS | died=77=tracked実dead77(counter整合) / 台帳241型(型集約の学習サンプル=died≧台帳は正常) |
...
| OP1 | 運用/パイプライン健全 | build_entities が成功して entity が新鮮(=合成パイプラインが crash してない) | ✅PASS | 最新player entity 0.3h前更新(133件)＝パイプライン稼働 |
