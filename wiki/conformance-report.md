...
| C4 | 指針4 | 1ソース取込→複数ページに波及(synthesis) | ✅PASS | 波及指示あり・summary3+entity215枚 |
| C5 | 指針5 | 矛盾は消さず⚠️両論で保持する | ✅PASS | concept内 ⚠️/矛盾 67箇所・prompt指示あり |
| C6 | 指針6 | entityで観測(事実)と推論(判断)を分離する | ✅PASS | 観測/判断を分離した entity 100/215枚 |
| C8 | 指針8 | bottom-up＝conceptを独断量産しない(動線/型が立つ時だけ) | ✅PASS | concept15枚(source7215・比0.002)・bottom-up痕17 |
...
| L2 | §Lint実挙動 | lintが実際に動いてる(stampが新しい＝存在でなく実行) | ✅PASS | last_lint 19h前(実行痕) |
...
| R3c | 指針3/6 死の計測整合 | 死の分母counter(base_rate.died)が実dead数(tracked.json)と一致＝正しく数えてる | ✅PASS | died=20=tracked実dead20(counter整合) / 台帳10型(型集約の学習サンプル=died≧台帳は正常) |
