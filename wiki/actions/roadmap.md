...
## P0 魔界edgeを"使える"状態に【finite・完了】
- [x] 🔧 **/check を bot配線**＝`/check <CA>`→ape/avoid判定（done）
- [x] 🔧 **/check v2**＝wiki合成知識(KOL track-record/型/Feedback)をlead・on-chainは足切りに降格（on-chain批判のfix・done）
- [x] 🔧 **KOL-entity lookup修正**＝`@<Handle>.md`照合に（信頼性injectが空回りしてたのを修正・done）
- [ ] ✅ 使い方の定着＝`/wiki <問い>`横断 / `/check <CA>`銘柄判定 / `/add`取り込み / 画像=vision

## P1 使う＋pullで厚く【ongoing・主フェーズ・一番効く・今ここ】
- [ ] ✅ 君が**実魔界プレイで /wiki・/check を叩く**（usageが corpus を複利化＝§Query資産化）
- [x] 🔧 **user-checked→tracking**＝君の /check が追加inputなしに死亡台帳/track-recordを厚く（done）
